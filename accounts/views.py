...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        # ===> ['.exist('] will return true or false
        if Account.objects.filter(email=email).exists():
            # ===> ['__exact'] will check if the email we are entering  is the same as the one in the database, ['__exact'] is case sensitive
            # ===> ['__iexact'] will be case insensitive
            ...
            messages.success(request, 'Password reset email has been sent to your meail address.')
            return redirect('login')
        else:
